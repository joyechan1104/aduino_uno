...
void setup() {
   // 초음파 센서 Vcc Trig Echo Gnd
    ...
    lcd.backlight(); // 백라이트
    lcd.setCursor(2,0); 
    lcd.print("Time:"); // 처리속도를 빠르게 하기위해 setup에 둔다.
    lcd.setCursor(2,1); // lcd.print는 띄우두면 화면을 유지하는 성질이 있기 때문이다.
    lcd.print("Dist:");  // ex) 숫자의 단위가 변하면 안지워지는 성질
    lcd.setCursor(13,0); 
    lcd.print("us"); 
    lcd.setCursor(11,1); 
    lcd.print("cm"); 
...
void loop() {
    ...
    Serial.print("Time :"); // Serial.print는 데이터가 휘발성이라서 loop에 넣어줘야한다.
